...
int Socket::Init(const char *IPAddr) {
  // Create the socket: AF_INET = IPv4, SOCK_DGRAM = UDP, and 0 is for default flags
  ourSocketDescriptor = socket(AF_INET, SOCK_DGRAM, 0);
  if (ourSocketDescriptor < 0) {
...
int Socket::SetSockaddrIn(struct sockaddr_in *addr, const char *ip, uint16_t port) {
  ...
  return inet_pton(AF_INET, ip, &(addr->sin_addr)); // set port number (host to network short)
}
...
char *Socket::ReceiveMessage() {
  ...
  bytesRecieved =
      recvfrom(ourSocketDescriptor, messageBuffer, BUFFERSIZE, MSG_DONTWAIT,
               (struct sockaddr *)&remoteAddr, &sizeof(myAddr));

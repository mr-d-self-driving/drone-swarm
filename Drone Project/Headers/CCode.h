...
//Sets up the socket with the given IP address and hard coded port. Returns 1 on success and 0 on error.
int Init(const char * IPAddr)
{

	//Create the socket, AF_INET = IPv4 and SOCK_DGRAM = UDP. The final parameter is for setting flags.
	...
	inet_pton(AF_INET, IPAddr, &(myAddr.sin_addr)); //Set IP and makes sure it has the network endian-ness
	myAddr.sin_port = htons(PORTNUM);				//Set Port number, htons() sets endian-ness

	//Bind the socket created earlier to the given myAddr info.
	if (bind(ourSocketDescriptor, (struct sockaddr *)&myAddr, sizeof(myAddr)) < 0)
	{
		perror("bind failed");
		return 0;
	}
...
int SetSendTo(const char * IPAddr)
{
	...
	memset((char *)&remoteAddr, 0, sizeof(remoteAddr));		        //Clear the memory, just in case
	remoteAddr.sin_family = AF_INET;						        //IPv4
	status = inet_pton(AF_INET, IPAddr, &(remoteAddr.sin_addr)); 	//Set IP and makes sure it has the network endian-ness
	remoteAddr.sin_port = htons(PORTNUM);					        //Set Port number, htons() sets endian-ness
	return status;
}
...
int SendMessage(char * message)
{
	...
	// sockaddr is a generic version for ipv4 and ipv6 so we can cast our struct to it
	if (sendto(ourSocketDescriptor, message, strlen((char*)message), 0, (struct sockaddr *)&remoteAddr, sizeof(remoteAddr)) < 0) {

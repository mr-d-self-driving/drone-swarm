...
class Vector3D {
 public:
  ...
  Vector3D UnitVector();
  Vector3D RotateZ(
      double angle);  // Rotate the vector in the z axis 'angle' degrees
  Vector3D Project(const Vector3D& onto);

  static std::vector<Vector3D> Repmat(Vector3D* vectorToRepeat,
                                      int timesToRepeat);
...
Vector3D operator-(const Vector3D& lhs,
                   const Vector3D& rhs);  // Returns a vector pointing towards
                                          // the rhs coordinate
